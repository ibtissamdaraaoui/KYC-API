...
def provision_client_on_kong(application_id: str) -> str:
    ...
    """
    logging.info(f"--- Provisionnement KONG pour '{application_id}' ---")
    try:
        ...
        logging.info(f"-> Consumer '{application_id}' OK.")
        ...
        if response.status_code == 201: # Si la credential vient d'être créée
            logging.info(f"-> Nouvelle JWT Credential pour '{application_id}' créée.")
            secret = response.json().get('secret')
        elif response.status_code == 409: # Si elle existait déjà, on la récupère
            logging.info(f"-> JWT Credential pour '{application_id}' existe déjà. Récupération...")
        ...
        logging.info(f"--- Provisionnement KONG pour '{application_id}' terminé. ---")
        return secret

    except requests.exceptions.RequestException as e:
        logging.info(f"❌ ERREUR KONG: Impossible de contacter l'API Admin de Kong. {e}")
        raise